...
public class ExerciciosMap {
	...
	public static void main(String[] args) {
		...
		System.out.println("\nExiba a soma da população de todos os estados:");
		Double soma = 0d;
		for (Map.Entry<String, Double> entry : maioresPopulacoes) {
			soma += entry.getValue();
		}
		...
		System.out.println("\nExiba a média da população dos estados:\n"+soma/maioresPopulacoes.size());
		
		System.out.println("\nRemova os estados com a população menor que 4000000:");
		Iterator<Double> iterator = estadosPopulacoes1.values().iterator();				
		while(iterator.hasNext()) {
			if(iterator.next() < 4000.00)
				iterator.remove();
		}
		for (Map.Entry<String, Double> entry : maioresPopulacoes) {
			System.out.println(entry);
		...
		System.out.println("\nApague o dicionário:");
		estadosPopulacoes1.clear();
		System.out.println(estadosPopulacoes1);
		
		System.out.println("\nConfira se o conjunto está vazio:\n"+estadosPopulacoes1.isEmpty());
